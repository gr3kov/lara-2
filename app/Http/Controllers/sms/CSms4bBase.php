...
';
    ...
    function CSms4bBase($login = '', $password = '')
    {
        ...
        $this->gmt = "10";   // Ваш часовой пояс относительно всемирного координированного времени
...
", $arrHeader) . "\r\n\r\n" . $xml;
    ...
    protected function SaveMessage($xml)
    {
        $xml = str_replace("\r\n", '', $xml);
        $xml = str_replace("\n", '', $xml);
        $result = preg_match("/<SaveMessageResult>([0-9]+?)<\/SaveMessageResult>/", $xml, $find);

        $saveMessageResult = intval($result);
...
$type_of_encoding:
 ...
 1-UTF16*/
    ...
    protected function AnalyzeResultSaveMessage($rezult)
    {
        switch ($rezult) {
            case 0:
                $this->LastError = "Неразрешенная попытка доступа";
                return;
            case -1:
                $this->LastError = "Неверный логин или пароль";
                return;
            case -2:
                $this->LastError = "Потеря сеанса связи";
                return;
            case -3:
            case -4:
            case -5:
            ...
            case -18:
            case -19:
                $this->LastError = "Cбой выполнения веб-метода. Обратитесь в ТП";
                return;
            case -20:
                $this->LastError = "Cбой сеанса связи";
                return;
            case -21:
                $this->LastError = "Cообщение уже подтверждено";
                return;
            case -22:
                $this->LastError = "Неверный идентификатор сообщения";
                return;
            case -30:
                $this->LastError = "Неизвестная кодировка сообщения";
                return;
            case -31:
                $this->LastError = "Неразрешенная зона тарификации";
                return;
            case -50:
                $this->LastError = "Неверный отправитель";
                return;
            case -52:
                $this->LastError = "Недостаточно средств на Вашем счете";
                return;
            case -68:
                $this->LastError = "Пользователь заблокирован";
                return;
