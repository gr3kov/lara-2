...
<p>В этом примере дата последнего считывания sms хранится в переменной сессии. Правильным решением при реализации будет
    хранение даты в базе данных. Считываются входящие sms с даты приёма последней считанной sms.</p>

<?php $props = array("SessionId" => $SMS4B->GetSID(),
    //флаг выгрузки sms
    "Flags" => "0",
    /*"Flags"	=> "1",*/
);
...
foreach ($Massive as $arIndex):?>
    <?php $arIndex = array_reverse($arIndex);
    ...
    <div style="background-color:#F0F0F0;margin-top: 7px; width:1100px;">
        <table class="table">
            ...
            <?php for ($i = 0; $i < $numb; $i++): ?>
                <tr>
                    <td class="tdn"><?php =$postrow[$i]["S"]?></td>
                    <td class="tdn"><?php =$postrow[$i]["D"]?></td>
                    <td class="tdn"><?php =$postrow[$i]["M"]?></td>
                    <td class="tdn"><?php =$postrow[$i]["A"]?></td>
                    <td class="tdnt"><?php =$SMS4B->decode($postrow[$i]["B"], $postrow[$i]["E"])?></td>
                </tr>
            <?php endfor; ?>
            </tbody>
        </table>
