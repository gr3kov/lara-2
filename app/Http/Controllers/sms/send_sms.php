...
<?php if ($SMS4B->LastError == '' && $SMS4B->GetSOAP("AccountParams", array("SessionID" => $SMS4B->GetSID())) === true) {
    if (!$_REQUEST['apply']) {
    ...
    <form name="form1" method="POST" action="#">
        <table cellpadding="9px">
            <tr>
                ...
                <td>
                    <select name="sender_number">
                        <?php foreach ($arResult["ADRESSES"] as $arIndex): ?>
                            <?php if ($arIndex == "" || $arIndex == " "): ?>
                                <?php continue; ?>
                            <?php endif; ?>
                            <option value='<?php =$arIndex?>'
                                <?php if ($sender == $arIndex): ?> selected <?php endif; ?>><?php =$arIndex?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </td>
            ...
            <tr>
                ...
                <td>
                    <div class="counters">Количество набраных номеров <span class="fontColor" id='freeNumbers'>0</span>
                    </div>
                    ...
                    ><?php =($arResult["RESULT_MESSAGE"]["TYPE"] == "ERROR" ? $dest : '')?></textarea>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <textarea rows='6' id="message" name="message"
                              ...
                              cols="50"
                              rows="4"><?php =($arResult["RESULT_MESSAGE"]["TYPE"] == "ERROR" ? $mess : '')?></textarea>
                    <br/>
                    <span style="color: #486DAA; cursor:pointer; text-decoration:underline;"
                          onclick="document.getElementById('message').value = trans(document.getElementById('message').value); changetarea(document.getElementById('message'),document.getElementById('lengmess'));">Траснлитирировать в латиницу</span>
                    <br/>
                    <span style="color: #486DAA; cursor:pointer; text-decoration:underline;"
                          onclick="document.getElementById('message').value = trans_lat_to_kir(document.getElementById('message').value); changetarea(document.getElementById('message'),document.getElementById('lengmess'));">Траснлитирировать в киррилицу</span>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <input type="text" class="typeinput" id='BEGIN_SEND_AT' name="BEGIN_SEND_AT" size="20"
                           value="<?php =$_REQUEST['BEGIN_SEND_AT'] ? $_REQUEST['BEGIN_SEND_AT'] : date("d-m-Y H:i:s")?>"/>
                </td>
            </tr>

            <tr>
                <td>
                    <input type=checkbox id='ACTIVE_DATE_ACTUAL1' name="ACTIVE_DATE_ACTUAL" value="Y"
                           onclick="activeNightTimeNsEvent('ACTIVE_DATE_ACTUAL1','DATE_ACTUAL1','');"
                        <?php if ($_REQUEST["ACTIVE_DATE_ACTUAL"] == "Y"): ?> checked <?php endif; ?> />
                ...
                <td>
                    <input type="text" class="typeinput" id='DATE_ACTUAL1' name="DATE_ACTUAL" size="20"
                           value="<?php =$_REQUEST['DATE_ACTUAL'] ? $_REQUEST['DATE_ACTUAL'] : date("d-m-Y H:i:s", time() + 3600)?>"/>
                </td>
            </tr>

            <tr>
                <td>
                    <input type=checkbox id="ACTIVE_NIGHT_TIME_NS1" name="ACTIVE_NIGHT_TIME_NS" value="Y"
                           onclick="activeNightTimeNsEvent('ACTIVE_NIGHT_TIME_NS1','DATE_FROM_NS1','DATE_TO_NS1');"
                        <?php if ($_REQUEST["ACTIVE_NIGHT_TIME_NS"] == "Y"): ?> checked <?php endif; ?> />
                ...
                <td>
                    <select id='DATE_FROM_NS1'
                            name="DATE_FROM_NS" <?php if ($_REQUEST["ACTIVE_NIGHT_TIME_NS"] != "Y"): ?> disabled <?php endif; ?>>
                        <?php
                        ...
                        <?php for ($i = 0; $i < 24; $i++): ?>
                            <option
                                value="<?php =chr(65+$i)?>" <?php if (chr(65 + $i) == $checked_symbol_date_from_ns): ?> selected <?php endif; ?> ><?php =$i?>
                                :00
                            </option>
                        <?php endfor; ?>
                    </select>
                    ...
                    по
                    &nbsp;
                    <select id='DATE_TO_NS1'
                            name="DATE_TO_NS" <?php if ($_REQUEST["ACTIVE_NIGHT_TIME_NS"] != "Y"): ?> disabled <?php endif; ?> >
                        <?php
                        if (!isset($_REQUEST["ACTIVE_NIGHT_TIME_NS"]) || !isset($_REQUEST["apply"])) {
                        ...
                        <?php for ($i = 0; $i < 24; $i++): ?>
                            <option
                                value="<?php =chr(65+$i)?>" <?php if (chr(65 + $i) == $checked_symbol_date_to_ns): ?> selected <?php endif; ?> ><?php =$i+1?>
                                :00
                            </option>
                        <?php endfor; ?>
                    </select>
            ...
            <input type="hidden" name='checking_f5' value="<?php =md5(time())?>"/>
    </form>
</div>
