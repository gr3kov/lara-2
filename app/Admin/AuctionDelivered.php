...
AdminSection::registerModel(\App\Models\AuctionDelivered::class, function (ModelConfiguration $model) {
    $model->setTitle('Отправленные лоты');
    $model->enableAccessCheck();
    // Display
    $model->onDisplay(function () {
        $display = AdminDisplay::datatables()->setDisplaySearch(true);
        $display->setFilters(AdminDisplayFilter::field('isDelivered')->setValue(1)->setTitle('Отправлены'));
        $display->setColumns([
