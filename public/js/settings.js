...
country_value.forEach((element) => {
  element.textContent = document.querySelector(
    `.user-country-option[value="${country_select.value}"]`
  ).textContent;
});
...
if (modal_btns) {
  modal_btns.forEach((element) => {
    element.addEventListener("click", function (e) {
      ...
      cardexp_input = modal_wrapper.querySelector(
        "#settings-cardchange--cardexp"
      );
      if (cardexp_input) {
